...
- **Partial refresh + daily full refresh**: uses the partial-update LUT for normal refreshes (faster, lower power); at 00:00 every day one refresh switches to the full-update LUT to clear ghosting (as required by the vendor after many partial refreshes).
